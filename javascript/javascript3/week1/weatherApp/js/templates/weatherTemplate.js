  ...
  return `<div class="weather__wrapper">
      <div class="weather__content">
        <div class="temperature__wrapper">
          ...
          <div class="measuring">
            <input id="temp-c-${weatherData.id}" type="radio" class="temperature__system" name="temperature" checked />
            <label for="temp-c-${weatherData.id}" id="label-c-${weatherData.id}" class="temperature__label checked">C</label> 
            <input id="temp-f-${weatherData.id}" type="radio" class="temperature__system" name="temperature" />
            <label for="temp-f-${weatherData.id}" id="label-f-${weatherData.id}" class="temperature__label">F</label> 
          </div>
        </div>
        <div class="searched__city">
          <h3>${weatherData.name}</h3>
        </div>
        <div class="more-info">
          <div class="wind df align-items_c gap">
            <img src="./img/icons/wind.png" alt="" class="icon" />
            <span>wind speed: ${weatherData.wind.speed}</span>
          </div>
          <div class="cloudy df align-items_c gap">
            <img src="./img/icons/clouds.png" alt="" class="icon" />
            <span>${weatherData.weather[0].description}</span>
          </div>
          <div class="df align-items-c gap">
